...
You can send orderflow directly to any BuilderNet node you control or have access to. Flashbots also offers public endpoints for interacting with the network.

The global endpoint (`rpc.buildernet.org`) automatically routes requests to an available BuilderNet node. To target a specific region (e.g. US or EU), use the corresponding regional endpoint.

In all cases, submitted orderflow is propagated to other nodes in the network via the [Orderflow Proxy](https://buildernet.org/docs/os-services-builds#orderflow-proxy).

### Public BuilderNet endpoints
