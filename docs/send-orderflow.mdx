...
See also [this Golang code](https://github.com/flashbots/go-utils/blob/main/examples/send-multioperator-orderflow/main.go) for an full example about how to send a transaction with a pinned server certificate.

The code below shows how to get the builder certificate (`builder-cert.pem`) over a TDX-attested channel.
