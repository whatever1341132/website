...
docker-shell:
	docker run -p 3000:3000 --name builderhub-docs --rm -it -w /mnt -v $(CUR_DIR):/mnt node:20 /bin/bash

docker-deps:
	docker run -p 3000:3000 --name builderhub-docs --rm -it -w /mnt -v $(CUR_DIR):/mnt node:20 /usr/local/bin/npm install
